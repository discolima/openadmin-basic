$(document).ready(function(){
	...
	$("#gridIngresos").jqGrid({
        ...
        ondblClickRow: function(rowid) {
			if(rowid){
				var div = rowid.split('_');
				var form = document.sendto;
				var data = $(this).jqGrid('getRowData',rowid);
				if(data.status=="timbrada"){
					form.folio.value = div[1];
					form.mes.value = $('#varmes').val();
					form.anio.value = div[0];
					$(form).attr('action',base_url+'ingresos/download');
					...
					form.mes.value = $('#varmes').val();
					form.anio.value = div[0];
					$(form).attr('action',base_url+'ingresos/timbrar');
					form.submit();
				}
			}
        },
...
function cancelar(){
	...
	if(rowid){
		...
		if(data.status=='timbrada'){
			...
			form.mes.value = $('#varmes').val();
			form.anio.value = div[0];
			$(form).attr('action',base_url + "ingresos/cancelar");
			$(form).submit();
		} else msg("Esta factura no esta timbrada","warning");
	}
}
